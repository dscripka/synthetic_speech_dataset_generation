...
def slerp(ratio, t1, t2):
    '''
        ...
        Args:
            ...
            t2: Tensor2
    '''
    low_norm = t1 / torch.linalg.norm(t1, axis=1, keepdims=True)
    high_norm = t2 / torch.linalg.norm(t2, axis=1, keepdims=True)
    omega = torch.arccos((low_norm * high_norm).sum(1))
    so = torch.sin(omega)
    res = (torch.sin((1.0 - ratio) * omega) / so)[0] * t1 + (torch.sin(ratio * omega) / so)[0] * t2
    return res
...
class Flowtron(torch.nn.Module):
    ...
    def infer(self, residual, speaker_vecs, text, temperature=1.0,
        ...
        ids = speaker_vecs
        if ids.shape[-1] > 1:
            speaker_vecs = self.speaker_embedding(ids[0][0][None][None]).squeeze(0)
            for i in ids[0][1:]:
                speaker_vecs = slerp(0.5, speaker_vecs, self.speaker_embedding(i[None][None]).squeeze(0))
            speaker_vecs = speaker_vecs[None,]
        else:
            speaker_vecs = self.speaker_embedding(ids) 
