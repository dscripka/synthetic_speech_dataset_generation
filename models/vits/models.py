...
# Spherical interpolation (obtained from https://huggingface.co/spaces/flax-community/t5-vae/blob/d7ff34c05059690ad3fe124d52cb07538a6a822f/app.py)
def slerp(ratio, t1, t2):
    '''
        ...
        Args:
            ...
            t2: Tensor2
    '''
    low_norm = t1 / np.linalg.norm(t1, axis=1, keepdims=True)
    high_norm = t2 / np.linalg.norm(t2, axis=1, keepdims=True)
    omega = np.arccos((low_norm * high_norm).sum(1))
    so = np.sin(omega)
    res = (np.sin((1.0 - ratio) * omega) / so)[0] * t1 + (np.sin(ratio * omega) / so)[0] * t2
    return res
